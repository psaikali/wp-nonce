...
class Nonce_Field_Test extends Nonce_Test_Case {
	...
	public function test_creation_returns_a_field_markup_containing_nonce() {
		...
		$this->assertContains( $nonce_field->getNonce(), $generated_field );
		$this->assertContains( $nonce_field->getKey(), $generated_field );
	}
	...
	/**
	 * Test that the isValid( $value ) method will look for values in REQUEST if $value is not set
	 */
	public function test_creation_validity_without_value() {
		$nonce_field     = new Nonce_Field( 'some-action', 'some-key' );
		$generated_field = $nonce_field->create();

		$_REQUEST[ 'some-key' ] = $nonce_field->getNonce();

		$this->assertValidNonce( $nonce_field->isValid() );
	}
	...
	public function test_creation_validity_with_value() {
		...
		$this->assertValidNonce( $nonce_field->isValid( $generated_nonce ) );
	}
}
